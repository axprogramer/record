...
function saveData(){
    ...
    // Data location Name
    localStorage.myReport = JSON.stringify(studentArray);
...
//Restore Data back
function getData(){
    if (localStorage.myReport){
        document.getElementsByName('tbody').innerHTML = "";
        studentArray = JSON.parse(localStorage.myReport);
...
let myReport = JSON.parse(localStorage.getItem("myReport"));
function updateQuantity(myId, week,date,pay,other){
	for(let product of myReport){
	...
	localStorage.setItem("myReport", JSON.stringify(myReport));
}
function updateData(){
    var week = document.getElementById("myWeek").value;
    var date = document.getElementById("myDate").value;
