const firebaseConfig = {
  ...
  var recordB = firebase.database().ref('recordB');
...
function selectAllData(){
  ...
  studentN0=0;
  firebase.database().ref('recordB').once('value',
  function(AllRecords){
    AllRecords.forEach(
      function(CurrentRecord){
        ...
        addItemsToTable(week, date,pay,other);
      }
    );
  });
}
...
var studentN0;
var stdList = [];
function addItemsToTable(week,date,pay,other){
  ...
  stdList.push([week,date,pay,other]);
...
function DelStdAll(){
  firebase.database().ref("recordB").remove();
